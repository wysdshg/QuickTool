...
- **来源窗口**：`_handle_drag_end` 在拖选结束瞬间记下前台窗口句柄与标题（与热键路径一致的做法），便签段头的来源标注、以及「回搜」切回原窗口才有正确落点。
...
*QuickTool v1.6.4 · 运行时零第三方依赖 · 底层能力基于 Win32 API（RegisterHotKey / 剪贴板 / Shell_NotifyIcon）*
